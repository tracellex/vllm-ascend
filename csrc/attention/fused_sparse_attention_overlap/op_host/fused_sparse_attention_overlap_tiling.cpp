...
void FusedSparseAttentionOverlapMlaTiling::InitParams()
{
    ...
    //
    // ⚠⚠ A/B 对比版（2026-08-26）：SPLIT_KV_MAX_BS=0 且 kernel 侧 helper 激活块
    //   已禁用 —— splitKV 与 helper 全档关闭，量「窗口化批量搬运 + opt01-05 + FD」
    //   单独的贡献。完整分工策略版（bs≤6 splitKV / 6<bs≤12 helper，实测归档后
    //   定档）见 commit 09cec9b8，恢复只需还原那两处。
    // splitKVFlag_ 恒 false 时 tiling key bit0 = 0，与基线逐位相同 —— 同一编译
    // 变体、同一 workspace 尺寸，被测差异只剩 kernel 代码本身。
    constexpr uint32_t SPLIT_KV_MAX_BS = 0U;   // A/B 对比（2026-08-26）：置 0 = 全档关闭 splitKV
    splitKVFlag_ = (perfMode_ == FusedSparseAttentionOverlapPerfMode::V_TEMPLATE_MODE) &&
                   (sfaInfo_->qLayout == FusedSparseAttentionOverlapLayout::TND) &&
                   (sfaInfo_->bSize > 0) && (sfaInfo_->bSize <= SPLIT_KV_MAX_BS) &&
                   (sfaInfo_->bSize * 4U <= aicNum_);
    kvSplitPart_ = 1U;
    if (splitKVFlag_) {
