...
class AscendKimiGatedDeltaNetAttention(KimiGatedDeltaNetAttention):
    ...
    @staticmethod
    def _resolve_kda_config(config) -> dict:
        """Return a dict with KDA parameters, supporting both old dict
        (linear_attn_config) and new flat-attr config formats."""
        if hasattr(config, 'linear_attn_config') and config.linear_attn_config is not None:
            return config.linear_attn_config
        # New flat-attr format
        return {
            "head_dim": getattr(config, 'linear_head_dim', 128),
            "num_heads": getattr(config, 'linear_num_heads', 64),
            "short_conv_kernel_size": getattr(config, 'linear_conv_kernel_dim', 4),
    ...
    def __init__(self, config, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        kda_config = self._resolve_kda_config(config)
        self.use_full_rank_gate = bool(kda_config.get("use_full_rank_gate", False))
        gate_lower_bound = kda_config.get("gate_lower_bound", kda_config.get("lower_bound"))
        ...
        if KimiK3TextConfig is not None and isinstance(config, KimiK3TextConfig):
